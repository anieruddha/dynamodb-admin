...
function loadDynamoConfig(env, AWS) {
  ...
  if (AWS.config) {
    if (!env.AWS_REGION && AWS.config.region !== undefined) {
      dynamoConfig.region = AWS.config.region
    }

    if (AWS.config.credentials) {
      if (!env.AWS_ACCESS_KEY_ID && AWS.config.credentials.accessKeyId !== undefined) {
...
const createAwsConfig = (AWS, cmdArgs) => {
  const env = cmdArgs || process.env
...
exports.createServer = (cmdArgs, dynamodb, docClient) => {
  const app = express()
  ...
  if (!dynamodb || !docClient) {
    ...
    if (!dynamodb) {
      dynamodb = new AWS.DynamoDB(createAwsConfig(AWS, cmdArgs))
    }
